...
public class KeyController : MonoBehaviour {
	...
	void Update () {
        if (!doorOpen) {
            if (doorInitialDirection.y == 1 && door.transform.position.y >= initialDoorPos.y + doorSize.y + doorSize.y)
            {
                stopDoor();
            }
            else if (doorInitialDirection.y == -1 && door.transform.position.y <= initialDoorPos.y - doorSize.y - doorSize.y)
            {
                stopDoor();
            }
            else if (doorInitialDirection.x == 1 && door.transform.position.x >= initialDoorPos.x + doorSize.x + doorSize.x)
            {
                stopDoor();
            }
            else if (doorInitialDirection.x == -1 && door.transform.position.x <= initialDoorPos.x - doorSize.x - doorSize.x)
